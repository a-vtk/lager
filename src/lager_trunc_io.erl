...
print(Term, _Max, #print_options{force_strings=true, lists_as_strings = LAS}) when not LAS,  not is_list(Term), not is_binary(Term), not is_atom(Term) ->
    erlang:error(badarg);
...
print(Bin, Max, _Options) when is_binary(Bin), Max < 2 ->
    {"<<...>>", 7};
print(Binary, Max, OldOptions) when is_binary(Binary) ->
    {B,Options} = try
                      {lager_format:cdata_to_chars(Binary),OldOptions}
    catch
        _:_  -> {binary_to_list(Binary, 1, lists:min([Max, byte_size(Binary)])),OldOptions}
    end ,
...
list_printing_test() ->
    ?assertEqual("[]", lists:flatten(format("~p", [[]], 50))),
    ?assertEqual("[{}]", lists:flatten(format("~tp", [[{}]], 50))),
        ...
        true ->
            ...
            ?assertEqual("#{22835963083295358096932575511191922182123945984 => bar}",
                         lists:flatten(format("~w", [
                                                     maps:from_list([{22835963083295358096932575511191922182123945984,
                                                       bar}])], 54))),
            ?assertEqual("#{bar => карамба}", lists:flatten(format("~tp", [maps:from_list([{bar, "карамба"}])], 100))),
            ok;
        false ->
            ok
    end.
...
unicode_test() ->
    ...
    ?assertEqual([31169], lists:flatten(format("~tp", [<<231,167,129>>], 50))),
    ok.
